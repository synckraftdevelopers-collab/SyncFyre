...
export const resourceSchemas = {
  "membership-plans": z.object({ branch_id: uuid.optional().nullable(), name: z.string().min(2).max(120), price: z.coerce.number().nonnegative(), gst_percent: z.coerce.number().min(0).max(100).default(18), discount_percent: z.coerce.number().min(0).max(100).default(0), duration_months: z.coerce.number().int().positive(), features: z.array(z.string()).default([]), plan_type: z.enum(["individual","couple"]).default("individual"), status }),
  subscriptions: z.object({ member_id: uuid, plan_id: uuid, branch_id: uuid, start_date: z.string().date(), end_date: z.string().date().optional(), status: z.enum(["pending","active","expired","cancelled","paused"]).optional(), auto_renew: z.boolean().optional(), price: z.coerce.number().nonnegative(), discount_amount: z.coerce.number().nonnegative().default(0), gst_amount: z.coerce.number().nonnegative().default(0), total_amount: z.coerce.number().nonnegative(), linked_subscription_id: uuid.optional().nullable() }),
