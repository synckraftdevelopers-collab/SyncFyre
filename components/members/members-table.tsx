  ...
  if (!data.length) return (
    <div className="grid min-h-64 place-items-center text-center">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Add your first member or adjust the filters.</p>
      </div>
    </div>
  );
      ...
      <table className="w-full min-w-[900px] text-sm">
        <thead className="border-b bg-muted/50 text-left text-xs uppercase text-muted-foreground">
          {table.getHeaderGroups().map(g => (
            <tr key={g.id}>{g.headers.map(h => <th key={h.id} className="px-4 py-3 font-medium">{flexRender(h.column.columnDef.header, h.getContext())}</th>)}</tr>
          ))}
        </thead>
