...
export function AddMemberWizard({ branches, plans, trainers, memberFormFields, basePath = "/admin/members", initialBranchId = null }: AddMemberWizardProps) {
  ...
  const maxDiscount = Math.max(0, planPrice - planDiscount);
  const discountAmount = Math.min(maxDiscount, Math.max(0, enteredDiscount));
  const discountedPrice = planPrice - planDiscount - discountAmount;
  const gstAmt = Math.round((discountedPrice * (selectedPlan?.gst_percent ?? 18)) / 100);
  const totalAmt = discountedPrice + gstAmt;
  ...
  const hasEmergencyStep = sections.emergency.length > 0;
  const wizardSteps = [
    { key: "personal", label: "Personal", icon: User, fields: sections.personal.map((field) => field.key as keyof WizardFormData) },
    ...(hasEmergencyStep
    ...
    { key: "save", label: "Save", icon: Save, fields: [] as Array<keyof WizardFormData> },
  ] as const;
  const activeStep = wizardSteps[step - 1];
  ...
  async function goNext() {
    ...
    if (valid) setStep((current) => Math.min(current + 1, wizardSteps.length));
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="flex items-center gap-0 overflow-x-auto pb-2">{wizardSteps.map((item, idx) => { const Icon = item.icon; const done = step > idx + 1; const current = step === idx + 1; return <div key={item.key} className="flex items-center"><button type="button" disabled={idx + 1 > step} onClick={() => idx + 1 < step && setStep(idx + 1)} className={cn("flex flex-col items-center gap-1 rounded-xl px-3 py-1.5 transition-all", current && "bg-primary/10", done && "cursor-pointer opacity-80 hover:bg-muted", idx + 1 > step && "cursor-not-allowed opacity-40")}><div className={cn("grid size-8 place-items-center rounded-full border-2 transition-all", current && "border-primary bg-primary text-white", done && "border-emerald-500 bg-emerald-500 text-white", idx + 1 > step && "border-border bg-background text-muted-foreground")}>{done ? <Check className="size-4" /> : <Icon className="size-4" />}</div><span className="whitespace-nowrap text-[10px] font-medium">{item.label}</span></button>{idx < wizardSteps.length - 1 ? <div className={cn("h-0.5 w-6 flex-shrink-0 rounded-full transition-colors", done ? "bg-emerald-500" : "bg-border")} /> : null}</div>; })}</div>
