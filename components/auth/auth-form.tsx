...
export function AuthForm({ action, mode }: { action: (state: AuthState, data: FormData) => Promise<AuthState>; mode: "login" | "forgot" | "reset" }) {
  ...
  return <div className="w-full max-w-sm">
    ...
    <form action={formAction} className="mt-7 space-y-5">
      ...
      {mode !== "forgot" && <label className="block text-sm font-medium">Password<Input name="password" type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} className="mt-2" placeholder={mode === "login" ? "Enter your password" : "Create a password"} required /></label>}
