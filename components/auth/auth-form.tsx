...
export function AuthForm({ action, mode }: { action: (state: AuthState, data: FormData) => Promise<AuthState>; mode: "login" | "forgot" | "reset" | "register" }) {
  ...
  return <div className="w-full max-w-md">
    ...
    <form action={formAction} className="mt-7 space-y-5">
      ...
      {mode === "login" && <p className="pt-2 text-center text-xs leading-5 text-muted-foreground">By signing in, you agree to our<br /><Link href="/terms" className="font-medium text-foreground hover:underline">Terms</Link>{" "}and{" "}<Link href="/privacy" className="font-medium text-foreground hover:underline">Privacy</Link>.</p>}
