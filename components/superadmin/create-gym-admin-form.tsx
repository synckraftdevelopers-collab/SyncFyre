...
export function CreateGymAdminForm() {
  ...
  return <form action={action} className="grid gap-4 md:grid-cols-2">
    ...
    <label className="text-sm font-medium">Administrator name<Input name="admin_name" className="mt-1.5" placeholder="Full name" required /></label>
    <label className="text-sm font-medium">Administrator email<Input name="email" type="email" className="mt-1.5" placeholder="admin@example.com" required /></label>
    ...
    <div className="md:col-span-2"><Button disabled={pending}>{pending ? <LoaderCircle className="size-4 animate-spin" /> : <Plus className="size-4" />} Create gym and administrator</Button></div>
  </form>;
}