...
export function MachineAttendanceTerminal({ device }: { device: TerminalDevice }) {
  ...
  async function submit() {
    ...
    try {
      const response = await fetch("/api/machine/attendance", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ deviceId: device.device_id, biometricUserId: memberId.trim(), eventType }) });
      const body = await response.json(); const status = body.result?.status as string | undefined;
  ...
  return <main className="grid min-h-screen bg-slate-950 p-4 text-slate-100 sm:p-8"><section className="m-auto w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-700 bg-slate-900 shadow-2xl"><header className="flex items-center justify-between border-b border-slate-700 px-6 py-5"><div className="flex items-center gap-3"><ScanFace className="size-8 text-cyan-400" /><div><h1 className="text-xl font-bold">SyncFyre Attendance</h1><p className="text-sm text-slate-400">{device.machine_name} · Face & biometric terminal</p></div></div><div className="flex items-center gap-2 text-sm text-slate-300"><RadioTower className={`size-4 ${device.connection_status === "online" ? "text-emerald-400" : "text-amber-400"}`} />{device.connection_status}</div></header><div className="space-y-6 p-6 sm:p-8"><div className="rounded-2xl border border-dashed border-slate-600 bg-slate-800/60 px-6 py-10 text-center"><ScanFace className="mx-auto mb-4 size-16 text-cyan-400" /><h2 className="text-2xl font-semibold">Ready to identify member</h2><p className="mt-2 text-sm text-slate-400">Use the connected face-recognition device, or enter a biometric ID for assisted check-in.</p></div><label className="block text-sm font-medium">Member biometric ID<Input value={memberId} onChange={(event) => setMemberId(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void submit(); }} className="mt-2 border-slate-600 bg-slate-800 text-white" placeholder="Scan or enter member ID" autoFocus /></label><div className="grid grid-cols-2 gap-3"><Button type="button" variant={eventType === "check_in" ? "default" : "outline"} onClick={() => setEventType("check_in")}>Check in</Button><Button type="button" variant={eventType === "check_out" ? "default" : "outline"} onClick={() => setEventType("check_out")}>Check out</Button></div><Button type="button" className="h-12 w-full" disabled={pending || !memberId.trim()} onClick={() => void submit()}>{pending ? <LoaderCircle className="animate-spin" /> : <ScanFace />}{pending ? "Verifying…" : "Verify and mark attendance"}</Button>{result && <div role="status" className={`flex gap-3 rounded-xl border p-4 ${result.success ? "border-emerald-700 bg-emerald-950/50 text-emerald-100" : "border-rose-700 bg-rose-950/50 text-rose-100"}`}>{result.success ? <CheckCircle2 className="mt-0.5 size-5 shrink-0" /> : <CircleAlert className="mt-0.5 size-5 shrink-0" />}<div><p className="font-semibold">{result.title}</p><p className="text-sm opacity-90">{result.message}</p></div></div>}</div></section></main>;
}