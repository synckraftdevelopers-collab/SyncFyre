...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <>
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-[#071d38] text-white shadow-2xl transition-transform lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
      <div className="flex h-24 items-center border-b border-white/10 px-6">
        <div className="rounded-xl bg-white px-3 py-1.5">
          <Image src="/syncfyre-logo.png" width={154} height={74} alt="SyncFyre" className="h-12 w-auto object-contain" priority />
        </div>
        <button onClick={onClose} className="ml-auto rounded-lg p-2 text-white/70 hover:bg-white/10 lg:hidden" aria-label="Close"><X className="size-5" /></button>
      </div>
      ...
      <div className="m-4 rounded-2xl border border-white/10 bg-white/5 p-4">
        ...
        <p className="mt-1 text-[11px] leading-relaxed text-white/45">Every member. Every payment. Every moving part.</p>
