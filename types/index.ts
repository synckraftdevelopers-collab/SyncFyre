...
export interface GstTransaction { id: string; branch_id: string; tenant_id?: string | null; payment_id?: string | null; invoice_id?: string | null; member_id?: string | null; txn_type: "sales" | "purchase"; reference_type: "income" | "expense" | "payment"; reference_id: string; invoice_number: string | null; party_name: string | null; party_gstin: string | null; taxable_amount: number; gst_rate?: number; gst_type?: "none" | "intra" | "inter"; cgst_rate: number; sgst_rate: number; igst_rate: number; cgst_amount: number; sgst_amount: number; igst_amount: number; total_tax: number; hsn_sac: string | null; txn_date: string; status: FinTxnStatus; created_by: string | null; created_at: string; updated_at: string; members?: { full_name: string; member_code: string } | null; branches?: { name: string } | null; payments?: { amount: number; method: string; status: string } | null; }
export interface Receivable { id: string; branch_id: string; member_id: string | null; invoice_id: string | null; subscription_id: string | null; receivable_type: ReceivableType; original_amount: number; paid_amount: number; balance_amount: number; due_date: string | null; status: ReceivableStatus; is_installment: boolean; next_installment_due_date: string | null; reminder_count: number; last_reminder_at: string | null; notes: string | null; created_by: string | null; updated_by: string | null; created_at: string; updated_at: string; members?: { full_name: string; member_code: string; phone: string } | null; }
...
export interface FinanceDashboardMetrics { selectedCollection: number; totalRevenue: number; totalExpenses: number; netProfit: number; cashInHand: number; bankBalance: number; outstandingReceivables: number; gstCollected: number; cgstCollected: number; sgstCollected: number; igstCollected: number; membershipRevenue: number; activeMembers: number; membershipsRenewingDue: number; collectionEfficiency: number; avgRevenuePerMember: number; periodStart?: string; periodEnd?: string; }
...
export interface GstParams extends FinanceParams { txnType?: "sales" | "purchase" | "all"; financialYear?: string; }
