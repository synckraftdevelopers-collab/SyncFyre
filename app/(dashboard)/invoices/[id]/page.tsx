...
export default async function InvoiceDetailPage({
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <Card className="print:shadow-none" id="printable-invoice">
                    ...
                    {invoice.line_items.map((item, i) => (
                      <tr key={i}>
                        <td className="px-4 py-2">
                          {String((item as Record<string, unknown>).description ?? "Item")}
                        </td>
                        <td className="px-4 py-2 text-right tabular-nums">
                          {formatCurrency(Number((item as Record<string, unknown>).amount ?? 0))}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {p.transaction_reference && (
                          <> &middot; <span className="font-mono">{p.transaction_reference}</span></>
                        )}
                      </p>
                    </div>
      ...
      <div className="hidden print:block text-center text-xs text-muted-foreground border-t pt-4">
        SyncFyre Gym Management &mdash; Thank you for your payment.
      </div>
    </div>
  );
}
