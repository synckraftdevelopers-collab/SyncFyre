...
export async function updateMemberAction(_: MemberFormState, formData: FormData): Promise<MemberFormState> {
  await requireUser(["admin", "manager", "reception"]);
  ...
  if (!parsed.success)
    return { error: "Review the highlighted information.", fields: parsed.error.flatten().fieldErrors as Record<string, string[]> };
  try { await updateMember(id, parsed.data); }
  catch (error) { return { error: error instanceof Error ? error.message : "Unable to update member." }; }
...
export async function deleteMemberAction(id: string): Promise<{ error?: string }> {
  ...
  catch (error) { return { error: error instanceof Error ? error.message : "Unable to delete member." }; }
  revalidatePath("/members");
  redirect("/members");
}
...
export async function uploadMemberPhotoAction(
  ...
  try { await updateMember(id, { profile_photo_url: url } as Parameters<typeof updateMember>[1]); }
  catch (error) { return { error: error instanceof Error ? error.message : "Photo saved but profile not updated." }; }
