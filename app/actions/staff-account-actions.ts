...
export async function createStaffAccountAction(_: StaffAccountState, formData: FormData): Promise<StaffAccountState> {
  ...
  const { data: staffRecord, error: staffError } = await admin.from("staff").insert({ user_id: userId, branch_id: branchId, tenant_id: requester.tenant_id, employee_code: employeeCode, designation, status: "active" }).select("id").single();
