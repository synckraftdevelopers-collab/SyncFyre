...
export async function createPtPackageAction(formData: FormData) {
  ...
  if (!(await hasCurrentFeature("pt"))) return { error: "PT is not included in the current plan." };
...
export async function schedulePtSessionAction(formData: FormData) {
  ...
  if (!(await hasCurrentFeature("pt"))) return { error: "PT is not included in the current plan." }; if (!profile.tenant_id || !profile.branch_id) return { error: "Organization and branch are required." };
...
export async function completePtSessionAction(sessionId: string) { const profile = await requireUser(["owner", "admin", "manager", "reception", "trainer"]); if (!(await hasCurrentFeature("pt"))) return { error: "PT is not included in the current plan." }; const supabase = await createClient(); const { error } = await supabase.rpc("complete_pt_session", { p_session_id: sessionId, p_user_id: profile.id }); if (error) return { error: error.message }; refresh(); return { success: "Session completed and one credit used." }; }
