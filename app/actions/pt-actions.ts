...
export async function createPtPackageAction(formData: FormData) {
  ...
  const entitlement = await ensurePaidCommercialPlan(profile.tenant_id, "PT & Trainer Management");
...
export async function completePtSessionAction(sessionId: string) { const profile = await requireUser(["owner", "admin", "manager", "reception", "trainer"]); const entitlement = await ensurePaidCommercialPlan(profile.tenant_id, "PT & Trainer Management"); if (!entitlement.allowed) return { error: entitlement.error }; const supabase = await createClient(); const { error } = await supabase.rpc("complete_pt_session", { p_session_id: sessionId, p_user_id: profile.id }); if (error) return { error: error.message }; refresh(); return { success: "Session completed and one credit used." }; }
