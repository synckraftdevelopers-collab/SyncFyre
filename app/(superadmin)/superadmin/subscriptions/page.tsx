...
const SORTABLE_COLUMNS = [
  ...
  { label: "Gym branch", column: "branch" as const },
  { label: "Plan", column: "plan" as const },
  { label: "Term", column: "term" as const },
...
export default async function SuperAdminSubscriptionsPage({
  ...
  if (colSort) {
    ...
    const sortValue = (s: Subscription): string | number => {
      switch (colSort) {
        case "member": return s.members?.full_name ?? "";
        case "branch": return s.branches?.name ?? "";
        case "plan": return s.membership_plans?.name ?? "";
        case "term": return s.start_date ?? "";
        case "amount": return Number(s.total_amount) || 0;
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-bold">Subscriptions</h1><p className="text-sm text-muted-foreground">Live member membership subscriptions across all gyms.</p></div><Card><CardHeader className="flex-row items-center justify-between"><CardTitle>Membership subscriptions</CardTitle><Badge variant="secondary">{subscriptions.length} shown</Badge></CardHeader><CardContent className="overflow-x-auto">{subscriptions.length ? <table className="w-full min-w-[760px] text-sm"><thead className="border-b bg-muted/50 text-left text-xs uppercase text-muted-foreground"><tr>{SORTABLE_COLUMNS.map(({ label, column }) => <SortableTh key={column} label={label} column={column} basePath="/superadmin/subscriptions" searchParams={sp as Record<string, string | undefined>} currentSort={colSort} currentDir={colDir} paramNames={{ sort: "colSort", dir: "colDir" }} className="px-4 py-3 font-medium" />)}</tr></thead><tbody className="divide-y">{subscriptions.map((subscription) => <tr key={subscription.id}><td className="px-4 py-3"><p className="font-medium">{subscription.members?.full_name ?? "Unknown member"}</p><p className="text-xs text-muted-foreground">{subscription.members?.member_code ?? ""}</p></td><td className="px-4 py-3 text-muted-foreground">{subscription.branches?.name ?? "Unassigned"}</td><td className="px-4 py-3">{subscription.membership_plans?.name ?? "Unknown plan"}</td><td className="px-4 py-3 text-xs text-muted-foreground">{subscription.start_date} – {subscription.end_date}</td><td className="px-4 py-3">₹{Number(subscription.total_amount).toLocaleString("en-IN")}</td><td className="px-4 py-3"><Badge variant={subscription.status === "active" ? "success" : subscription.status === "cancelled" ? "danger" : "outline"}>{subscription.status}</Badge></td></tr>)}</tbody></table> : <p className="py-12 text-center text-sm text-muted-foreground">No subscriptions found.</p>}</CardContent></Card></div>;
}
