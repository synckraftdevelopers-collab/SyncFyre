...
const titles: Record<string, string> = {
  ...
  "face-machines": "Add face machine",
};
...
export default async function AdminNewResourcePage({ params }: { params: Promise<{ module: string }> }) {
  ...
  const fields: Record<string, FormField[]> = {
    ...
    "face-machines":[
      ...
      { name:"manufacturer",label:"Manufacturer",defaultValue:"eSSL" },
      { name:"model",label:"Model",defaultValue:"AiFace-ERIS" },
      { name:"serial_number",label:"Serial number" },
      { name:"device_id",label:"Device ID",required:true },
      { name:"device_identifier",label:"Device identifier" },
      { name:"connection_mode",label:"Connection mode",type:"select",defaultValue:"adms",options:[{label:"ADMS",value:"adms"},{label:"Push",value:"push"},{label:"Pull",value:"pull"},{label:"Unknown",value:"unknown"}] },
      { name:"machine_ip",label:"Machine IP" },
      { name:"allowed_ip",label:"Allowed source IP" },
      { name:"machine_api_url",label:"Optional pull API URL" },
      { name:"api_key_encrypted",label:"Optional shared secret / API key" },
      { name:"status",label:"Status",type:"select",defaultValue:"active",options:[{label:"Active",value:"active"},{label:"Inactive",value:"inactive"}] },
    ],
  };

  const resource = moduleKey === "memberships" ? "membership-plans" : moduleKey;
  const returnTo = moduleKey === "face-machines" ? "/admin/settings?tab=biometric" : `/admin/${moduleKey}`;
