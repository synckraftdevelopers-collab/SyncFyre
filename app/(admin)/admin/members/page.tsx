    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage profiles, health details, and membership status.</p>
        </div>
      ...
      <Card>
        <form className="flex flex-col gap-3 border-b p-4 sm:flex-row">
          <div className="relative flex-1">
            ...
            <Input name="q" defaultValue={query.q} className="pl-9" placeholder="Search by name, member ID, or phone" />
          </div>
          <select name="status" defaultValue={query.status ?? "all"} className="h-10 rounded-lg border bg-background px-3 text-sm">
        ...
        <MembersTable data={result.data} basePath="/admin/members" />

        <div className="flex items-center justify-between border-t p-4 text-sm text-muted-foreground">
          <span>{result.total} member{result.total === 1 ? "" : "s"}</span>
