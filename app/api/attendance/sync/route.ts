...
export async function POST(request: NextRequest) {
  ...
  for (const event of parsed.data.events) {
    const { data, error } = await supabase.rpc("process_attendance_event", {
      p_device_id: event.device_id, p_machine_user_id: event.machine_user_id, p_event_at: event.event_at,
      p_event_type: event.event_type, p_external_event_id: event.external_event_id, p_raw: event,
    });
